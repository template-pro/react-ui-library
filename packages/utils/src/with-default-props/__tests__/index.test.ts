...
describe('mergeProps', () => {
  it('should work', () => {
    ...
    expect(mergeProps({ foo: undefined }, { foo: 2 })).toEqual({ foo: 2 })
    expect(mergeProps({ foo: 1 }, { foo: undefined })).toEqual({ foo: 1 })
    ...
    expect(
      mergeProps({ foo: 1, bar: undefined }, { foo: undefined, bar: 2 }),
    ).toEqual({
      foo: 1,
      bar: 2,
    })
